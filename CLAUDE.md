...
The `stroke thickness` bullet in that `Preserve:` list was observed adding an outline/stroke to generated icons even when the selected style's reference image had none — the same class of problem as the app-icon black-corner issue above: sending the reference image inline is only a soft visual guide, and the bare word "stroke thickness" primes toward *having* a stroke, competing against Gemini's strong learned prior that icon-style imagery is typically outlined. The bullet was reworded to be explicit that "no stroke" is a valid match to preserve ("match the reference exactly, including having NO outline/stroke at all if the reference has none; do not add an outline that isn't present in the reference image") rather than changing the Preserve-list architecture itself.
...
Unlike resolution/corruption, blur and heavy subject cropping can't be checked deterministically, so the same `analyzeStyleImage()` vision call is also asked to judge them, returning a `quality: { blurry, heavilyCropped, reason }` field alongside the style characteristics. If either flag comes back true, `StyleModal.tsx#analyzeImage` throws a local `ImageQualityError`, which — unlike a generic analysis failure — is treated as blocking: the image is rejected outright (`uploadedImage` reset to `null`) and `quality.reason` is shown via `uploadError`, forcing the user to pick a different reference image rather than letting them save a blurry/heavily-cropped style anyway.
